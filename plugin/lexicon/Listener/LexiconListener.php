...
class LexiconListener
{
    ...
    private function desktop()
    {
      ...
      $data_content = '{}';
      $user = $this->post_user('toma', 'toto', '0222ssssss3', 'email');
      echo $user;
      $data_content = "{'.$user.'}";
    ...
    public function post_user($name, $login, $password, $email)
    {
        ...
        $response = $CLIENT_USER->request('POST', 'users/' . $login, ['body' => $userjsondata, 'auth' => [$login, $password], 'http_errors' => false]);
        $code = $response->getStatusCode();
        if ($code != 201) {
            $reason = $response->getReasonPhrase();
            echo "<p class='apierror'>REST APIUSERS POST USER ERROR: $code $reason</p>\n";
        }
