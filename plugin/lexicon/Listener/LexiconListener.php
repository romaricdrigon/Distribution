...
use Claroline\LexiconBundle\Manager\DictionariesUsersManager;
...
class LexiconListener
{
    ...
    {   
      $data_content = $this->dataJibiki()[0];
      $user         = $this->dataJibiki()[1];
    ...
    {
        $users        = $this->container->get('claroline_lexicon.manager.users');
        $dictionaries = $this->container->get('claroline_lexicon.manager.dictionaries');
        $alldata      = $dictionaries->getAllUserResources();
        $user         = $users->getCurrentUser();
        return array($alldata, $user);
    }
